...
//
// In general, please adhere to the following guidelines and ensure that
// all contributing authors follow them also:
// 1. Leave no empty sections.
// 2. Correct all misspelling according to idiomatic American English.
// 3. All acronyms must be defined the first time they are used.
// 4. Avoid domain-specific buzzwords.
// 5. Write in 3rd person voice; avoid using "we".
// 6. Ensure correct Grammar and use direct language.
// 7. Use a colon (:) to introduce bulleted lists.
// 8. Avoid pronouns with ambiguous references (the problem with "it").
//
...
Reference URL for this document: http://www.opengis.net/doc/PER/t14-{referenceURL}
